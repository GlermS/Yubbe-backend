...
import Theme = require('./models/Theme')
...
class MongoDB implements DataBaseInterface{
    ...
    async joinCall(userId: String, callId: String):Promise<Response>{
        ...
        if(response.isNotFull){
            ...
            if(isNotRegistered){
                const call = await CallModel.findOneAndUpdate({ _id: callId },{$addToSet:{clients:userId}})
                console.log(call)
            if(call.theme){
                resp = {code: 201 ,data: {user: await UserModel.findOne({_id:userId}),theme:await ThemeModel.findOne({_id:call.theme}),moderator:await UserModel.findOne({_id:call.moderatorId}),call}};
            }else{
    ...
    admCreateTheme = async (editorId:string, themeData: any)=>{
        dbConnect();
        var resp:Response;
        ...
        await UserModel.findOne({'_id':editorId}).then(async (editor)=>{
            if(editor.authorization==='adm'){
                ...
                resp= {code:200, data:await ThemeModel.create(theme)}
            }else{
                resp = defaultAnswer.unauthorized
            }
            }).catch((err) =>{
    ...
    admUpdateTheme = async (editorId:string, themeId:any, themeData: any)=>{
        dbConnect();
        var resp:Response;
        ...
        await UserModel.findOne({'_id':editorId}).then(async (editor)=>{
            if(editor.authorization==='adm'){
                ...
                resp= {code:200, data:await ThemeModel.updateOne({"_id":themeId},theme)}
            }else{
                resp = defaultAnswer.unauthorized
            }
            }).catch((err) =>{
    ...
    listThemes = async ()=>{
        ...
        await ThemeModel.find().then((themes)=>{
            resp= {code:200, data:themes}
        }).catch(err=>{resp= defaultAnswer.internalError})

        return resp
    }

    getThemeInfo = async (editorId:string, themeId:any)=>{
        dbConnect();
        var resp:Response;

        await ThemeModel.findOne({_id: mongoose.Types.ObjectId(themeId)}).then((theme)=>{
            resp= {code:200, data:theme}
        }).catch(err=>{resp= defaultAnswer.internalError})
